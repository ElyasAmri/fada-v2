"""Quick test of VILT-b32-finetuned-vqa on FADA dataset"""
...
print("="*70)
print("VILT-b32-finetuned-vqa Quick Test on FADA Dataset")
print("="*70)
...
print(f"\n1. Loading VILT-b32-finetuned-vqa model...")
...
try:
    ...
    print("="*70)
    print(f"\nVILT-b32:         {param_count / 1e9:.2f}B params, ~{final_memory if torch.cuda.is_available() else 'N/A':.1f}GB memory")
    ...
    print("\nNote: VILT is very lightweight, trained on VQAv2 dataset")
    print("Model predicts from fixed vocabulary (not generative)")

    # Save results
    import json
    with open("vilt_quick_test_results.json", "w") as f:
        json.dump(results, f, indent=2)

    print(f"\nResults saved to: vilt_quick_test_results.json")
