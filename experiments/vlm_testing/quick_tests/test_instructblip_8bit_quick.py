"""Quick test of InstructBLIP-Vicuna-7B with 8-bit quantization on FADA dataset"""
...
print("="*70)
print("InstructBLIP-Vicuna-7B (8-bit) Quick Test on FADA Dataset")
print("="*70)
...
print(f"\n1. Loading InstructBLIP-Vicuna-7B with 8-bit quantization...")
...
try:
    ...
    print("="*70)
    print(f"\nModel: InstructBLIP-Vicuna-7B (8-bit quantized)")
    print(f"Parameters: {param_count / 1e9:.2f}B")
    ...
    print("="*70)
    print(f"\nInstructBLIP-7B-8b: {param_count / 1e9:.2f}B params, ~{final_memory if torch.cuda.is_available() else 'N/A':.1f}GB memory")
    ...
    print("\nNote: InstructBLIP is instruction-tuned BLIP-2 with Vicuna-7B")
    print("Expected to have excellent VQA performance")

    # Save results
    import json
    with open("instructblip_8bit_quick_test_results.json", "w") as f:
        json.dump(results, f, indent=2)

    print(f"\nResults saved to: instructblip_8bit_quick_test_results.json")
