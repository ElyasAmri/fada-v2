"""Test InternVL2-2B - OpenGVLab's 2B multimodal model"""
...
try:
    ...
    for i, img_path in enumerate(test_images[:6]):
        ...
        # InternVL2 format - pass image path
        question = "This is a fetal ultrasound image. What anatomical structures can you identify? Provide a detailed medical description."

        # Generate
        start = time.time()
        with torch.no_grad():
            response = model.chat(tokenizer, str(img_path), question, generation_config=dict(max_new_tokens=200, do_sample=False))
    ...
    print("\n" + "="*70)
    print("InternVL2-2B Test Complete")
    print("="*70)

    overall_score = (fetal_context + correct_anatomy + medical_quality) / (total_tested * 3) * 100
    print(f"\nOverall: {overall_score:.1f}%")
    print(f"Better than Qwen2-VL-2B (~83.3%)? {'YES!' if overall_score > 83.3 else 'NO'}")
